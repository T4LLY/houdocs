...
def build_node_metadata(
    *,
    ...
    ports = [
        NodePort(node_type_id, "input", index, field.label, field.description)
        for index, field in enumerate(node_doc.inputs)
    ] + [
        NodePort(node_type_id, "output", index, field.label, field.description)
        for index, field in enumerate(node_doc.outputs)
    ]
