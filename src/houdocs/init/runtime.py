...
def _parse_runtime_node_type(value: object, *, index: int) -> RuntimeNodeSnapshot:
    ...
    return RuntimeNodeSnapshot(
        ...
        min_inputs=_optional_integer(value.get("min_inputs"), index=index, field="min_inputs"),
        max_inputs=_optional_integer(value.get("max_inputs"), index=index, field="max_inputs"),
        max_outputs=_optional_integer(value.get("max_outputs"), index=index, field="max_outputs"),
        parameters=tuple(
            _parse_runtime_parameter(parameter, node_index=index, parameter_index=parameter_index)
...
def _parse_runtime_parameter(
    ...
    *,
    ...
    if not isinstance(folder_path, list) or not all(isinstance(item, str) for item in folder_path):
